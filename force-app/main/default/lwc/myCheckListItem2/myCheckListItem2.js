...
export default class TaskList extends LightningElement {
    @api recordId;
    @track documents = [];
    @track cmsApprovalStatus;
    @track stateApprovalStatus;
    ...
    updateDocumentStatuses() {
        const cmsApprovalDoc = this.documents.find((doc) =>
            doc.title.includes('CMS Approval Letter')
        );
        const stateApprovalDoc = this.documents.find((doc) =>
            doc.title.includes('State Approval Letter')
        );

        this.cmsApprovalStatus = cmsApprovalDoc
            ? cmsApprovalDoc.status
            : 'NO FILE';
        this.stateApprovalStatus = stateApprovalDoc
            ? stateApprovalDoc.status
            : 'NO FILE';
    ...
    updateProgressBar() {
        let progress = 0;
        if (this.cmsApprovalStatus === 'APPROVED') {
            progress += 50;
        }
        if (this.stateApprovalStatus === 'APPROVED') {
            progress += 50;
        }
    ...
    handleFileChange(event) {
        ...
        uploadedFiles.forEach(file => {
            ...
            switch (documentType) {
                case 'cmsApproval':
                    documentTitle = `CMS Approval Letter${fileExtension}`;
                    break;
                case 'stateApproval':
                    documentTitle = `State Approval Letter${fileExtension}`;
                    break;
                default:
